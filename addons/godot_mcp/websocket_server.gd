...
var _logger := MCPLogger.new("WebSocket Server")
...
func start_server() -> int:
	...
	if err == OK:
		set_process(true)
		_logger.info("MCP WebSocket server started on port %d" % _port)
	else:
		_logger.error("Failed to start MCP WebSocket server: %d" % err)
...
func poll() -> void:
	...
	while not refuse_new_connections and tcp_server.is_connection_available():
		...
		assert(conn != null)
		_logger.info("New TCP connection, starting WebSocket handshake...")
	...
	for p in pending_peers:
		if not _connect_pending(p):
			if p.connect_time + handshake_timeout < Time.get_ticks_msec():
				# Timeout
				_logger.info("WebSocket handshake timed out")
				to_remove.append(p)
	...
	for id in peers:
		...
		if state == WebSocketPeer.STATE_CLOSING or state == WebSocketPeer.STATE_CLOSED:
			_logger.info("Client %d disconnected (state: %d)" % [id, state])
		...
		while p.get_available_packet_count() > 0:
			...
			if parse_result == OK:
				...
				_logger.debug("Received command from client %d: %s" % [id, command])
				emit_signal("command_received", id, command)
			else:
				_logger.error("Error parsing JSON from client %d: %s at line %d" %
...
func _connect_pending(p: PendingPeer) -> bool:
	if p.ws != null:
		...
		if state == WebSocketPeer.STATE_OPEN:
			...
			peers[id] = p.ws
			_logger.info("Client %d WebSocket connection established" % id)
			emit_signal("client_connected", id)
			return true # Success.
		elif state != WebSocketPeer.STATE_CONNECTING:
			_logger.info("WebSocket handshake failed, state: %d" % state)
			return true # Failure.
		return false # Still connecting.
	else:
		if p.tcp.get_status() != StreamPeerTCP.STATUS_CONNECTED:
			_logger.info("TCP connection lost during handshake")
			return true # TCP disconnected.
		else:
			# TCP is ready, create WS peer
			_logger.info("TCP connected, upgrading to WebSocket...")
